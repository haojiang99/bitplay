...
(async function ($) {
  ...
  const settingsModal = document.querySelector("#settings-model");
  ...
  // YTS Server Settings Save Handler
  document
    .querySelector("#yts-settings-form")
    .addEventListener("submit", async (e) => {
      e.preventDefault();
      const selectedServer = document.querySelector('input[name="ytsServer"]:checked');
      ...
      const body = {
        ytsServerUrl: selectedServer.value,
      };

      const response = await fetch("/api/v1/settings/yts", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(body),
      });
      ...
      if (!response.ok) {
        ...
        butterup.toast({
          message: "Movie server settings saved successfully",
          location: "top-right",
          icon: true,
          dismissable: true,
          type: "success",
        ...
        settings = {
          ...settings,
          ytsServerUrl: body.ytsServerUrl,
        };
      }

      submitButton.removeAttribute("disabled");
      submitButton.classList.remove("loader");
      submitButton.innerHTML = "Save Server Settings";
    });
  ...
  fetch("/api/v1/settings")
    ...
    .then((data) => {
      ...
      // Set YTS server selection
      const ytsServerUrl = data.ytsServerUrl || "https://yts.mx/api/v2/list_movies.json";
      const radioButtons = document.querySelectorAll('input[name="ytsServer"]');
      radioButtons.forEach((radio) => {
        if (radio.value === ytsServerUrl) {
          radio.checked = true;
        }
      });
